document.addEventListener('DOMContentLoaded', () => {
  ...
  const commands = {
    ...
    cd: function (args, currentDirectory, directories) {
      ...
      if (dir === '..') {
        ...
        directories[currentDirectory].includes(dir) &&
        directories.hasOwnProperty(
          `${currentDirectory === '/' ? '' : currentDirectory}/${dir}`,
        )
      ) {
        return {
          newDir: `${currentDirectory === '/' ? '' : currentDirectory}/${dir}`,
        }
      } else {
  ...
  const peopleLinks = {
    ...
    sergio: 'people/sergio/sergio.html',
  }
  ...
  function handleCommand(command) {
    ...
    if (commands[cmd]) {
      ...
      printText(
        `\nCommand ${command} not found. Try 'help' for a list of available commands.`,
        () => (isCommandProcessing = false),
      )
    }
  }
